...
func TestDefaultConfigUsesClaudeBoundary(t *testing.T) {
	config := runtimeproc.DefaultConfig()
	if config.Binary != "claude" {
		t.Fatalf("Binary = %q, want claude", config.Binary)
	}
	for _, name := range []string{"PATH", "ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL", "CLAUDE_CONFIG_DIR"} {
