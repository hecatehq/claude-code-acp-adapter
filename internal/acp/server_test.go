...
func TestInitializeResponse(t *testing.T) {
	...
	if info["name"] != "claude-code-acp-adapter" {
...
func TestInitializeCanUseRuntimeResult(t *testing.T) {
	server := NewServer(AdapterInfo{Name: "claude-code-acp-adapter"}, WithInitializeResult(map[string]any{
		"protocolVersion": 1,
...
func TestInitializeHandlerReceivesClientParams(t *testing.T) {
	server := NewServer(AdapterInfo{Name: "claude-code-acp-adapter"}, WithInitializeHandler(func(params json.RawMessage) (any, *RPCError) {
		var req struct {
...
func TestPromptReturnsStructuredNotImplemented(t *testing.T) {
	server := NewServer(AdapterInfo{Name: "claude-code-acp-adapter"})
...
func TestMethodContextRequestWaitsForClientResponse(t *testing.T) {
	server := NewServer(AdapterInfo{Name: "claude-code-acp-adapter"}, WithMethod("test/needs_client", func(ctx *MethodContext, _ json.RawMessage) (any, *RPCError) {
...
func TestNotificationDispatchesWhileMethodIsRunning(t *testing.T) {
	...
	server := NewServer(
		AdapterInfo{Name: "claude-code-acp-adapter"},
