...
func TestBuildEnvAllowlistAndOverrides(t *testing.T) {
	env, err := adapterprocess.BuildEnv([]string{
		...
		Set:     map[string]string{"HOME": "/sandbox/home", "CLAUDE_CONFIG_DIR": "/sandbox/claude"},
	})
	if err != nil {
		t.Fatalf("BuildEnv returned error: %v", err)
	}
	want := []string{"CLAUDE_CONFIG_DIR=/sandbox/claude", "HOME=/sandbox/home", "PATH=/bin"}
...
func TestRedactEnvAndArgs(t *testing.T) {
	...
	wantEnv := []string{"ANTHROPIC_API_KEY=[redacted]", "PATH=/bin", "SESSION_TOKEN=[redacted]"}
	...
	wantArgs := []string{"--api-key", "[redacted]", "--token=[redacted]", "--model", "sonnet"}
